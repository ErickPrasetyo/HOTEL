...
type
  ...
  TfrmPostingKolamRenang = class(TForm)
    pnlHeader: TPanel;
    advpnlNav: TAdvPanel;
...
var
  frmPostingKolamRenang: TfrmPostingKolamRenang;
...
procedure TfrmPostingKolamRenang.AdvGlBtnRefreshClick(Sender: TObject);
begin
  OpenDataset(pgLapResto.ActivePageIndex);
end;

procedure TfrmPostingKolamRenang.Batal1Click(Sender: TObject);
var s: string;
begin
   try
       Exec.Close;
...
constructor TfrmPostingKolamRenang.Create(AOwner: TComponent; aTag: Integer);
begin
  inherited Create(AOwner);
  vTag := aTag;
end;

procedure TfrmPostingKolamRenang.FormClose(Sender: TObject; var Action: TCloseAction);
begin
 Action:= caFree;
 frmPostingKolamRenang:=nil;
end;

procedure TfrmPostingKolamRenang.FormShow(Sender: TObject);
begin
...
procedure TfrmPostingKolamRenang.OpenDataset(idx:integer);
begin
  try
    case idx of
      0 : begin
...
procedure TfrmPostingKolamRenang.UnBatal1Click(Sender: TObject);
var s: string;
begin
   try
       Exec.Close;
...
procedure TfrmPostingKolamRenang.UpdateAccessControls;
...
procedure TfrmPostingKolamRenang.wwDbDtpFromCloseUp(Sender: TObject);
var dt: TDateTime;
    y,m,d: Word;
begin
...
procedure TfrmPostingKolamRenang.glbtnCLoseClick(Sender: TObject);
begin
  Close;
end;

procedure TfrmPostingKolamRenang.glbtnGoClick(Sender: TObject);
begin
  OpenDataset(pgLapResto.ActivePageIndex);
end;
